...
pub fn parse<'s, E: Error<'s>>(input: Input<'s>) -> IResult<Input<'s>, (), E> {
    branch::alt((
        parse_single,
    ))(input)
}
...
/// # Errors
/// This function errors if we failed to parse a comment for whatever reason. A `nom::Err::Error` means that it may be something else on top of there, but `nom::Err::Failure` means that the stream will never be valid.
pub fn parse_single<'s, E: Error<'s>>(input: Input<'s>) -> IResult<Input<'s>, (), E> {
    comb::value((), seq::pair(
        bc::tag("//"),
        seq::terminated(
            comb::opt(bc::is_not("\n")),
...
/// # Errors
/// This function errors if we failed to parse a comment for whatever reason. A `nom::Err::Error` means that it may be something else on top of there, but `nom::Err::Failure` means that the stream will never be valid.
pub fn parse_multi<'s, E: Error<'s>>(input: Input<'s>) -> IResult<Input<'s>, (), E> {
    comb::value((), seq::pair(
        bc::tag("/*"),
        comb::cut(seq::pair(
            bc::take_until("*/"),
