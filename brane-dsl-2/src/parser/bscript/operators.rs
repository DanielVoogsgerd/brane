...
#[inline]
pub(crate) fn unary<'t, 's>(input: Input<'t, 's>) -> IResult<Input<'t, 's>, UnaryOperator, Error<'t, 's>> {
    branch::alt((
...
/// # Errors
///  This function errors if we failed to parse an operator for whatever reason. A `nom::Err::Error` means that it may be something else on top of there, but `nom::Err::Failure` means that the stream will never be valid.
fn binary<'t, 's>(input: Input<'t, 's>) -> IResult<Input<'t, 's>, BinaryOperator, Error<'t, 's>> {
    branch::alt((
        // Arithmetic
        ...
        // Miscellaneous
        comb::map(tag_token!('t, 's, Token::Assign), |o| BinaryOperator{ kind: BinaryOperatorKind::Assign, range: Some(o.range()) }),
    ))(input)
}
...
pub(crate) fn postfix<'t, 's>(input: Input<'t, 's>) -> IResult<Input<'t, 's>, ExpressionPostfix, Error<'t, 's>> {
    branch::alt((
...
// ///  This function errors if we failed to parse an operator for whatever reason. A `nom::Err::Error` means that it may be something else on top of there, but `nom::Err::Failure` means that the stream will never be valid.
// #[inline]
// pub fn parse<'t, 's>(input: Input<'t, 's>) -> IResult<Input<'t, 's>, Operator, Error<'t, 's>> {
//     branch::alt((
